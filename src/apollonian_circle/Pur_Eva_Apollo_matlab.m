clear;close all;clc
V_E=0.2;V_P=0.15;  % 逃跑者E、追捕者P的速度
lamda=V_P / V_E;  % 速度比
P=[0,1; 0.6,0.2; 0.1,0.4; 0,-1; -0.4,-0.3; -0.6,-0.2];  % 追捕者位置（这里是6个）
E=[0,0];
N = length(P);  % 追捕者的个数
...
plt_limit = 2;
xlim([-plt_limit plt_limit]);ylim([-plt_limit plt_limit])
pic_num = 1;  % 画gif用
...
%             id_E(end+1,:) = i;flag_leak=1;  % 如果有多个缺口，都保存下来，
    ...
    if ~inpolygon(Et(1,t+1)',Et(2,t+1)',Pt(1,k,t+1)',Pt(2,k,t+1)')
        break;
    end
    pic_num = pic_num + 1;
end
